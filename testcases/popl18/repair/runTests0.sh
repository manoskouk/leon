...
function run {
  cmd="./leon --debug=report --timeout=90 --repair --introreccalls=off --solvers=fairz3,enum  --functions=$1 $2"
  echo "Running " $cmd
  echo "------------------------------------------------------------------------------------------------------------------"
  $cmd | tee -a $fullLog
}
...
# NEW
run semUntyped  testcases/popl18/repair/Compiler/Compiler7.scala
...
#NEW
run '&'       testcases/popl18/repair/List/List14.scala          
run count     testcases/popl18/repair/List/List15.scala          
