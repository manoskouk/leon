...
object Heaps {
  ...
  @production(1) def mkL(): Heap = Leaf()
  @production(1) def mkN(value: BigInt, left: Heap, right: Heap) = Node(value, left, right)
  @production(1) def useMakeN(value: BigInt, left: Heap, right: Heap) : Heap = makeN(value, left, right)
  @production(40) def useRank(h: Heap) = h.rank
